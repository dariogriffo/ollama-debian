...
# Function to map Ubuntu architecture to ollama release name
get_ollama_release() {
    local arch=$1
    case "$arch" in
        "amd64")
...
build_architecture() {
    ...
    for dist in "${arr[@]}"; do
        FULL_VERSION="$ollama_VERSION-${BUILD_VERSION}+${dist}_${build_arch}_ubu"
        echo "  Building $FULL_VERSION"

        if ! docker build . -f Dockerfile.ubu -t "ollama-ubuntu-$dist-$build_arch" \
        ...
        id="$(docker create "ollama-ubuntu-$dist-$build_arch")"
        ...
        docker rmi "ollama-ubuntu-$dist-$build_arch" > /dev/null || true
    done
...
if [ "$ARCH" = "all" ]; then
    ...
    # All supported architectures (Ubuntu, same as Debian for ollama)
