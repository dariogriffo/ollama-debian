...
if [ -z "$ollama_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    ...
    echo "Example: $0 0.31.2 1"
    exit 1
fi

PACKAGE_NAME="ollama"
ORIG_TARBALL="${PACKAGE_NAME}_${ollama_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${ollama_VERSION}"

echo "Creating Debian/Ubuntu source packages for ollama ${ollama_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/ollama/ollama/archive/refs/tags/v${ollama_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${ollama_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as ollama-0.x.y/ which matches our BUILD_DIR
...
ollama (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${ollama_VERSION}.
